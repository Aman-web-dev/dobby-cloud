...
import { Upload, FolderPlus, Search, Grid, List, Cloud, Star, Trash2 } from 'lucide-react';
...
const DobbyCloud = () => {
  ...
  const files = [
    { id: 5, name: 'vacation-photo.jpg', type: 'image', size: '2.3 MB', preview: 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=100&h=100&fit=crop' },
    { id: 6, name: 'presentation.pdf', type: 'document', size: '4.1 MB' },
    { id: 7, name: 'design-mockup.png', type: 'image', size: '1.8 MB', preview: 'https://images.unsplash.com/photo-1541462608143-67571c6738dd?w=100&h=100&fit=crop' },
  ...
  const MacFolder = ({ folder, isSelected, onClick }:any) => (
    ...
    >
      <div className="flex flex-col items-center p-4">
        ...
        <span className="text-sm text-gray-700 text-center max-w-20 truncate">{folder.name}</span>
      ...
      return (
        ...
        >
          <div className="flex flex-col items-center p-4">
            ...
            <span className="text-sm text-gray-700 text-center max-w-20 truncate">{file.name}</span>
          ...
          {viewMode === 'grid' ? (
            ...
            <div className="space-y-2">
              {allItems.map(item => (
                <div 
                  key={item.id}
                  className={`flex items-center p-3 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors ${selectedItems.has(item.id) ? 'bg-blue-50 border border-blue-200' : ''}`}
                ...
                >
                  <div className="w-8 h-8 mr-3 flex-shrink-0">
                    {item.type === 'folder' ? (
                      ...
                      ></div>
                    ) : item.type === 'image' && 'preview' in item && item.preview ? (
